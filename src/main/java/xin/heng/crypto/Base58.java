...
public class Base58 {
     ...
     *
     * @param wallet  the HXWallet to use sm3 digest to build a checksum.
     * @param payload the bytes to encode, e.g. pubkey hash
     * @return the base58-encoded string
     */
    public static String encodeChecked(HXWallet wallet, byte[] payload) {
        // A stringified buffer is:
        // 1 byte version + data bytes + 4 bytes check code (a truncated hash)
        byte[] addressBytes = new byte[payload.length + 4];
        System.arraycopy(payload, 0, addressBytes, 2, payload.length);
        byte[] checkBytes = Arrays.copyOfRange(addressBytes, 0, 2 + payload.length);
        byte[] checksum = wallet.digestBySM3(checkBytes);
